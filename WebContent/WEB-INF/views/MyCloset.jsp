...
$(document).ready(function(){
	...
	$("input[name=tabs]").change(function(e) {
		if(e.target.id == 'tab1'){
			...
			$(".outer-grid").html('<c:choose><c:when test="${fn:length(cartProductList) != 0}"><c:forEach var="i" begin="0" end="${fn:length(cartProductList) - 1}"><div class="inner-grid"><img src="upload/${cartProductList[i].files.files_name}"><div class="overlay"><a href="<%=request.getContextPath()%>/ProductDetailPage.Pd?prd_num=${cartProductList[i].prd_num}"><p id="hoverPrdTitle">${cartProductList[i].prd_title}</p></a><div class="hoverPrdPrice"><span>${cartProductList[i].prd_price_won}</span></div><div class="hoverPrdHeart"><c:choose><c:when test="${empty cartList}"><button class="far fa-heart like" value="${cartProductList[i].prd_num}"></button></c:when><c:otherwise><c:forEach var="cart" items="${cartList}"><c:if test="${cartProductList[i].prd_num != cart.prd_num}"><button class="far fa-heart like" value="${cartProductList[i].prd_num}"></button><span>${cartLikeList[i]}</span><span><i class="fas fa-comment"></i>&nbsp;5</span></c:if><c:if test="${cartProductList[i].prd_num == cart.prd_num}"><button class="fas fa-heart like" value="${cartProductList[i].prd_num}"></button><span style="margin-right: 10px;">${cartLikeList[i]}</span><span><i class="fas fa-comment"></i>&nbsp;5</span></c:if></c:forEach></c:otherwise></c:choose></div></div></div></c:forEach></c:when><c:otherwise><h6>찜한 상품이 없습니다.</h6></c:otherwise></c:choose>');
		}
	});
</script>
<script>
